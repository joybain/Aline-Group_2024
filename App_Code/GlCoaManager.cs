...
namespace Dorjibari
{
    public class GlCoaManager
    {
        ...
        public static DataTable getCoaBalanceFromExistingDataTableTrial(DataTable dt, string book, string glcoacode, string sepTyp, string status, string fromdt, string todt, string StartDt1, int? ReportType)
        {
            ...
            DataTable dtBlance = new DataTable();
            dtBlance.Columns.Add("Dr_Blance", typeof(decimal));
            dtBlance.Columns.Add("Cr_Blance", typeof(decimal));

            //string OpeningStock = IdManager.GetShowSingleValueString("Opening_Stock", "ID", "FixGlCoaCode", "1");
            DataTable dtFixCode = IdManager.GetShowDataTable("SELECT * FROM [FixValue]");
            ...
            foreach (DataRow dr in dtAlready.Rows)
            {
                ...
                //}
                if (dtFixCode.Rows[0]["ClosingStock"].ToString().Trim().Equals(dr["gl_coa_code"].ToString().Trim()))
                {
            ...
            DataTable dtdtl;
            decimal balDR = decimal.Zero;
            decimal balCR = decimal.Zero;
            if (fromdt == "")
            {
                        ...
                        foreach (DataRow drdtl in dtdtl.Rows)
                        {
                            ...
                            if (StartDt1 == null)
                            {
                                ...
                                if (DateTime.Parse(drdtl["value_date"].ToString()).CompareTo(DataManager.DateEncode(todt)) <= 0)
                                {
                                    ...
                                    balDR += Math.Ceiling(decimal.Parse(drdtl["amount_dr"].ToString()));
                                    balCR += Math.Ceiling(decimal.Parse(drdtl["amount_cr"].ToString()));
            ...
            dtAlready.Dispose();
            DataRow DrBal = dtBlance.NewRow();
            DrBal["Dr_Blance"] = balDR;
            DrBal["Cr_Blance"] = balCR;
            dtBlance.Rows.Add(DrBal);

            return dtBlance;
        }

        public static void CreateGlCoaForBank(GlCoa glcoa)
        {
            String connectionString = DataManager.OraConnString();
            SqlConnection sqlCon = new SqlConnection(connectionString);

            string query = "insert into gl_coa (book_name,gl_coa_code,coa_enabled,effective_from,effective_to, " +
                " bud_allowed,post_allowed,taxable,acc_type,status,coa_desc,coa_curr_bal,coa_natural_code,ENTRY_USER,ENTRY_DATE,AUTHO_USER,AUTHO_DATE,[AccountStatus]) values ( " +
                " '" + glcoa.BookName + "',  '" + glcoa.GlCoaCode + "',  '" + glcoa.CoaEnabled + "', " +
                "  convert(datetime,case '" + glcoa.EffectiveFrom + "' when '' then null else '" + glcoa.EffectiveFrom + "' end ,103), convert(datetime,case '" + glcoa.EffectiveTo + "' when '' then null else '" + glcoa.EffectiveTo + "' end ,103), '" + glcoa.BudAllowed + "', " +
                "  '" + glcoa.PostAllowed + "',  '" + glcoa.Taxable + "',  '" + glcoa.AccType + "', " +
                "  '" + glcoa.Status + "',  '" + glcoa.CoaDesc.Replace("'", "") + "',  '" + glcoa.CoaCurrBal + "', " +
                "  '" + glcoa.CoaNaturalCode + "','" + glcoa.LoginBy + "',GETDATE(),'" + glcoa.LoginBy + "',GETDATE(),'"+glcoa.AccountType+"') ";
        ...
        public static void UpdateGlCoa(GlCoa glcoa)
        {
            ...
            string query = "update gl_coa set gl_coa_code= '" + glcoa.GlCoaCode + "', coa_enabled= '" + glcoa.CoaEnabled + "', " +
                ...
                " post_allowed= '" + glcoa.PostAllowed + "',[AccountStatus]='" + glcoa.AccountType + "', taxable= '" + glcoa.Taxable + "', acc_type= '" + glcoa.AccType + "', " +
                ...
                foreach (DataRow drdtA in dtAlready.Rows)
                {
                    ...
                    if (dtdtl.Rows.Count > 0)
                    {
                        foreach (DataRow drdtl in dtdtl.Rows)
                        {
                            ...
                            try
                            {
                                 a = DateTime.Parse(drdtl["value_date"].ToString()).CompareTo(DataManager.DateEncode(fromdt));
                                 b = DateTime.Parse(drdtl["value_date"].ToString()).CompareTo(DataManager.DateEncode(todt));
                            }

                            catch
                            {
                                a = DateTime.Parse(drdtl["value_date"].ToString()).CompareTo(fromdt);
                                b = DateTime.Parse(drdtl["value_date"].ToString()).CompareTo(todt);
                            }
                            //if (DateTime.Parse(drdtl["value_date"].ToString()).CompareTo(DataManager.DateEncode(fromdt)) >= 0 &&
        ...
        public static void UpdateOpeningBalance(DataRow dr)
        {
            string connectionString = DataManager.OraConnString();
            SqlConnection sqlCon = new SqlConnection(connectionString);
            string query = @"UPDATE [GL_COA]
   SET coa_desc='" + dr["coa_desc"].ToString() + "',acc_type='" + dr["acc_type"].ToString() + "',status='" + dr["status"].ToString() + "', [opening_balance] ='" + dr["opening_balance"].ToString() + "' WHERE [GL_COA_CODE]='" + dr["GL_COA_CODE"].ToString() + "'";
            DataManager.ExecuteNonQuery(connectionString, query);
        }
