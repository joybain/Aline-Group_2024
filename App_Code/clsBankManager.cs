...
namespace Dorjibari
{
    public class clsBankManager
    {
        ...
        {
            String connectionString = DataManager.OraConnString();
            string query = "select bank_id,bank_name,Gl_Code,BankType,ParentCode from bank_info";
        ...
        {
            String connectionString = DataManager.OraConnString();
            string query = "SELECT [ID],[BANK_ID],[BRANCH_NAME],BankType,[ADDR1],BankCharge,[AccountType],[Active],[AccountName],[ADDR2],[ADDR3],[PHONE1],[PHONE2],[AccountNo],[Gl_Code]  FROM [dbo].[BANK_BRANCH] where  [ID]='" + ID + "' ";
            DataTable dt = DataManager.ExecuteQuery(connectionString, query, "BankDtl");
            if (dt.Rows.Count == 0)
            {
                return null;
        ...
        {
            String connectionString = DataManager.OraConnString();
            string query = @"Update [dbo].[PaymentMethod]  set [Name]='" + mst.Name + "',[LocalUploadBranchID]='0,',BranchID='"+mst.BranchID+"' ,UpdateBy='" + mst.LoginBy + "',UpdateDate=getdate(),Active='"+mst.Status+"',[LocalUploadFlag]=case when  LocalUploadFlag=1 then 2 else LocalUploadFlag end  where ID='" + mst.Id + "' ";
            DataManager.ExecuteNonQuery(connectionString, query);
        }

        public static void UpdatePaymentmethodMAnage(PaymentManageInfo mst)
        {
            String connectionString = DataManager.OraConnString();
            string query = @"Update [dbo].[PaymentMethodManage]  set [MethodName]='" + mst.Name + "',PaymentID='"+mst.PaymentID+"',[LocalUploadBranchID]='0,',BranchID='" + mst.BranchID + "' ,UpdateBy='" + mst.LoginBy + "',UpdateDate=getdate(),Active='" + mst.Status + "',[LocalUploadFlag]=case when  LocalUploadFlag=1 then 2 else LocalUploadFlag end  where ID='" + mst.Id + "' ";
            DataManager.ExecuteNonQuery(connectionString, query);
        }
        ...
        {
            String connectionString = DataManager.OraConnString();
            string query = @"INSERT INTO [dbo].[PaymentMethod]   ([Name],[Active],[LocalUploadFlag],AddBy,AddDate,BranchID)  VALUES ('" + mst.Name + "','" + mst.Status + "',0,'" + mst.LoginBy + "',GetDate(),'"+mst.BranchID+"')";
            DataManager.ExecuteNonQuery(connectionString, query);
        }

        public static void CreatePaymentmethodManage(PaymentManageInfo mst)
        {
            String connectionString = DataManager.OraConnString();
            string query = @"INSERT INTO [dbo].[PaymentMethodManage]   ([MethodName],[Active],[LocalUploadFlag],AddBy,AddDate,BranchID,PaymentID)  VALUES ('" + mst.Name + "','" + mst.Status + "',0,'" + mst.LoginBy + "',GetDate(),'" + mst.BranchID + "','"+mst.PaymentID+"')";
            DataManager.ExecuteNonQuery(connectionString, query);
        }
        ...
        {
            String connectionString = DataManager.OraConnString();
            string query = " select t1.ID, t1.Name,case when t1.Active=1 then 'Active' else 'In Active' end as Active from PaymentMethod t1  where  t1.DeleteBy is null order by id desc";
            DataTable dt = DataManager.ExecuteQuery(connectionString, query, "BankMaster");
            return dt;
        }

        public static object GetPaymentMethodInfoManages()
        {
            String connectionString = DataManager.OraConnString();
            string query = " select t1.ID, t2.Name,PaymentID,t1.BranchID,t3.BranchName,case when t1.Active=1 then 'Active' else 'In Active' end as Active from PaymentMethodManage t1  inner join PaymentMethod t2 on t2.ID=t1.PaymentID inner join BranchInfo t3 on t3.ID=t1.BranchID where  t1.DeleteBy is null order by PaymentID asc";
            DataTable dt = DataManager.ExecuteQuery(connectionString, query, "BankMaster");
            return dt;
        }
        ...
        public static PaymentManageInfo GetPaymentMethodManageInfodtls(string mst)
        {
            String connectionString = DataManager.OraConnString();
            string query = " select ID, [MethodName],PaymentID,[BranchID] from PaymentMethodManage where ID='" + mst + "' and DeleteBy is null ";
            DataTable dt = DataManager.ExecuteQuery(connectionString, query, "BankMaster");
            if (dt.Rows.Count == 0)
            {
                return null;
            }
            return new PaymentManageInfo(dt.Rows[0]);
        }
        ...
        public static void DeletePaymentMethodManage(PaymentManageInfo mst)
        {
            String connectionString = DataManager.OraConnString();
            string query = @"Update [dbo].[PaymentMethodManage]  set DeleteBy='" + mst.LoginBy + "',DeleteDate=getdate(),[LocalUploadFlag]=case when  LocalUploadFlag=1 then 2 else LocalUploadFlag end where ID='" + mst.Id + "'";
            DataManager.ExecuteNonQuery(connectionString, query);
        }
    }
}