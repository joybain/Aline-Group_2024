 ...
 ([Code],[Name],[ItemSize],[ItemColor],[UOMID],[UnitPrice],[Currency],[OpeningStock],[OpeningAmount],[ClosingStock],[ClosingAmount],[CategoryID],[SubCategoryID],[TaxCategoryID],[Discounted],[DiscountAmount],[Active],[IsNew],[AddBy],[AddDate],ItemImage,description,Brand,ShortName,StyleNo,SecurityCode,ItemsType,BranchSalesPrice)
     VALUES
           ('" + aClsItemDetailsInfoObj.ItemsCode + "','" + aClsItemDetailsInfoObj.ItemsName + "','0','0','" +
                        ...
                        "',GETDATE(),@img,'" + aClsItemDetailsInfoObj.Description + "','" + aClsItemDetailsInfoObj.Brand +
 ...
 ([Code],[Name],[ItemSize],[ItemColor],[UOMID],[UnitPrice],[Currency],[OpeningStock],[OpeningAmount],[ClosingStock],[ClosingAmount],[CategoryID],[SubCategoryID],[TaxCategoryID],[Discounted],[DiscountAmount],[Active],[IsNew],[AddBy],[AddDate],ItemImage,description,Brand,ShortName,StyleNo,SecurityCode,ItemsType,BranchSalesPrice)
     VALUES
           ('" + aClsItemDetailsInfoObj.ItemsCode + "','" + aClsItemDetailsInfoObj.ItemsName + "','0','0','" +
                        ...
                        aClsItemDetailsInfoObj.DiscountCheck + "','" + aClsItemDetailsInfoObj.Discount + "','" +
                        aClsItemDetailsInfoObj.Active + "','True','" + aClsItemDetailsInfoObj.LoginBy +
                        "',GETDATE(),null,'" + aClsItemDetailsInfoObj.Description + "','" + aClsItemDetailsInfoObj.Brand +
                        "','" + aClsItemDetailsInfoObj.ShortName + "','" + aClsItemDetailsInfoObj.StyleNo + "','" +
                        aClsItemDetailsInfoObj.SecurityCode + "','" + aClsItemDetailsInfoObj.ItemsType + "','" +
                        aClsItemDetailsInfoObj.BranchSalesPrice + "')";
            }
            ...
            string stockQuery = "";
            if (aClsItemDetailsInfoObj.ItemsType == "5")
            {
                stockQuery = @"INSERT INTO [ItemStock]
          (ItemID,Code, StyleNo, BatchNo, UnitPrice, Quantity,Total, CreatedBy, CreatedDate,ItemType,MsrUnitCode)
     VALUES
           (" + ItemsID + ",'" + aClsItemDetailsInfoObj.ItemsCode + "','" + aClsItemDetailsInfoObj.ItemsCode + "','0','" + aClsItemDetailsInfoObj.UnitPrice + "','" +
                       aClsItemDetailsInfoObj.AssestQnty + "',(" + aClsItemDetailsInfoObj.UnitPrice + " * " +
                       aClsItemDetailsInfoObj.AssestQnty + "),'" +
                       aClsItemDetailsInfoObj.LoginBy + "',GETDATE(),'" +
                       aClsItemDetailsInfoObj.ItemsType + "','" +
                        aClsItemDetailsInfoObj.Umo + "')";
                SqlCommand command3 = new SqlCommand(stockQuery, sqlCon);
                sqlCon.Open();
...
FROM  [Item] where ([Code]+' - '+[Name]) LIKE '%" +p+"%' and [items_type]='1'";
    ...
    public static DataTable GetShowItemsInfo(string SR)
    {
        string connectionString = DataManager.OraConnString();
        string Parameter="";
        if (SR != "") { Parameter = " AND UPPER(Items) LIKE '" + SR + "' "; }
        string selectQuery = @"
Select ID,Code,Name,UnitPrice AS UnitPrice,ClosingStock as Roll,SalesClosingStock AS ClosingStock,Convert(decimal(18,2),UnitPrice*SalesClosingStock) ClosingAmount ,UOM
from dbo.View_StockSearchInfo where UnitPrice>0 and isnull(ClosingStock,0)>0 and isnull(SalesClosingStock,0)>0  " + Parameter;
    ...
    public static DataTable GetShowStockItemsInfo(string SR)
    {
        string connectionString = DataManager.OraConnString();
        string Parameter = "";
        if (SR != "") { Parameter = " AND UPPER((Code+'_'+Name)) LIKE '" + SR + "' "; }
        string selectQuery = @"
Select ID,Code,Name,UnitPrice AS UnitPrice,ClosingStock as Roll,SalesClosingStock AS ClosingStock,Convert(decimal(18,2),UnitPrice*SalesClosingStock) ClosingAmount ,UOM
from dbo.View_ManufactureStockForSales where UnitPrice>0 and isnull(SalesClosingStock,0)>0 and ClosingStock>0 and ItemsType!=1 " + Parameter;
        //if (SR != "") { Parameter = " AND UPPER(t3.Code+' - '+t3.Name+' - '+Isnull(t1.BrandName,'')+' - '+Isnull(t2.Name,'')) LIKE '" + SR + "' "; }
        //string selectQuery = @"SELECT t3.ID,t3.Code,t3.Name,t.Price AS UnitPrice,t.Quantity AS ClosingStock,t3.ClosingAmount FROM ItemSalesStock t left join Item t3 on t3.ID=t.ItemsID left join Brand t1 on t3.Brand=t1.ID left join Category t2 on t2.ID=t3.CategoryID where t.Price>0 " + Parameter;
        DataTable dt = DataManager.ExecuteQuery(connectionString, selectQuery, "Item");
        return dt;
    ...
    public static DataTable  GetShowItemsDetails(string Item, string SearchType)
    {
        ...
        string parameter = "";
            ...
            if (Item != "")
            {
                parameter = " where convert(decimal(18,0),t1.[ClosingStock])>0 and upper(Items)=upper('" + Item + "')  and t1.Active='True' and t1.DeleteBy IS NULL ";
            }
            ...
            else if (Item == "" && SearchType != "0")
            {
                ...
                parameter = " Where convert(decimal(18,0),t1.[ClosingStock])>0 and t1.Active='True' and t1.DeleteBy IS NULL   and ItemsType='" + SearchType + "' ";

            }        

            else
            {
                parameter = " Where convert(decimal(18,0),t1.[ClosingStock])>0 and t1.Active='True' and t1.DeleteBy IS NULL ";
            }

            string selectQuery = @"SELECT t1.ID,Items ,Items+'-'+isnull(Date,'') as ItemsName 
      , Catagory
      ,SubCat
      ,[UnitPrice]     
      ,[OpeningStock]
	  ,ClosingStock as Roll
      ,convert(decimal(18,2),[OpeningAmount]) OpeningAmount   
      ,convert(decimal(18,2),isnull(CFTClosingStock,ClosingStock)) ClosingStock
	   ,convert(decimal(18,2),isnull(SFTClosingStock,ClosingStock)) SFTClosingStock
	    ,convert(decimal(18,2),isnull(SQMClosingStock,SQMClosingStock)) SQMClosingStock
      ,Convert(decimal(18,2),UnitPrice*SalesClosingStock) AS [ClosingAmount],UOM ,ProjectId,Case when ProjectId is Null then 'HeadOffice'
	  else  t2.ProjectName  end as ProjectName
  FROM View_StockSearchInfo t1 left join Project_Setup_Tbl t2 on t1.ProjectId=t2.Id " + parameter + " ORDER BY convert(decimal(18,0),t1.[ClosingStock]) desc, t1.[Items] ASC";
        DataTable dt = DataManager.ExecuteQuery(connectionString, selectQuery, "Item");
        return dt;
    }
...
left join ShiftmentAssigen t7 on t7.ID=t1.ItemsID "+ parameter;
  ...
  left join Brand t4 on t4.ID=t1.Brand where t1.DeleteBy IS NULL  " + Parameter + "  order by t1.ID DESC";
    ...
    public static DataTable GetShowItemsDetailsproject(string Item, string SearchType, string ProjectID)
    {
        string connectionString = DataManager.OraConnString();
        string parameter = "";
        ...
        if (Item != "")
        {
            parameter = " where convert(decimal(18,0),t1.[ClosingStock])>0 and upper(Items)=upper('" + Item + "') and ProjectId='" + ProjectID + "' and t1.Active='True' and t1.DeleteBy IS NULL ";
        }
        ...
        else if (ProjectID != "")
        {
            ...
            parameter = " Where convert(decimal(18,0),t1.[ClosingStock])>0 and upper(ProjectId)=upper('" + ProjectID + "') and t1.Active='True' and t1.DeleteBy IS NULL";
      ...
      ,convert(decimal(18,2),isnull(CFTClosingStock,ClosingStock)) ClosingStock
	   ,convert(decimal(18,2),isnull(SFTClosingStock,ClosingStock)) SFTClosingStock
	    ,convert(decimal(18,2),isnull(SQMClosingStock,SQMClosingStock)) SQMClosingStock
      ,Convert(decimal(18,2),UnitPrice*SalesClosingStock) AS [ClosingAmount]      ,UOM ,ProjectId,t2.ProjectName
  FROM View_StockSearchInfo t1 inner join Project_Setup_Tbl t2 on t1.ProjectId=t2.Id " + parameter + " ORDER BY convert(decimal(18,0),t1.[ClosingStock]) desc, t1.[Items] ASC";
        DataTable dt = DataManager.ExecuteQuery(connectionString, selectQuery, "Item");
        return dt;
    }
