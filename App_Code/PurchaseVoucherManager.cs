...
public class PurchaseVoucherManager
{
    ...
    public static PurchaseVoucherInfo GetPurchaseMst(string ID)
    {
        ...
        string query = @"SELECT t1.[ID],t1.[GRN],CONVERT(NVARCHAR,t1.[ReceivedDate],103) as ReceivedDate,t1.[PO],CONVERT(NVARCHAR,t1.[PODate],103) as PODate,t1.[ChallanNo],CONVERT(NVARCHAR,t1.[ChallanDate],103) as ChallanDate,t1.[SupplierID],t1.[Remarks],ISNULL(t1.[Total],0)Total,ISNULL(t1.[Discount],0) as Discount,t1.[CarriagePerson],t1.[LaburePerson],ISNULL(t1.[OtherCharge],0) as OtherCharge,ISNULL(t1.[CarriageCharge],0) as CarriageCharge,ISNULL(t1.[LabureCharge],0) as LabureCharge,ISNULL(t1.[TotalPayment],0) as TotalPayment,t1.ShiftmentID,'' as ShiftmentNO,t1.PartyID
,CASE WHEN t2.PayMethod='Q' and (t2.Chk_Status='P' OR t2.Chk_Status='B') THEN isnull(t1.[Total],0) else (isnull(t1.[Total],0)-isnull(t1.Discount,0)-isnull(t1.[TotalPayment],0)) end AS Due
,CASE WHEN t2.PayMethod IS NULL THEN 'C' ELSE t2.PayMethod END AS [PaymentMethod],t2.Bank_id AS[BankName],t2.[ChequeNo],CONVERT(NVARCHAR,t2.[ChequeDate],103) as ChequeDate ,t1.[ChequeAmount],t2.Chk_Status FROM [ItemPurchaseMst] t1 left join SupplierPayment t2 on t2.purchase_id=t1.ID and t2.Payment_Type='PV'  Where t1.ID='" + ID + "' ";
    ...
    public static int SavePurchaseVoucher(PurchaseVoucherInfo purmst, DataTable dt,string RequjationId, string ORID,string PaymentCoa)
    {
        SqlConnection connection = new SqlConnection(DataManager.OraConnString());
        SqlTransaction transaction;
        ...
        try
        {
           ...
           ([GRN],[ReceivedDate],[PO],[PODate],ProjectId,ProjectSite,[ChallanNo],[ChallanDate],[SupplierID],[Remarks],[Total],Discount,[CreatedBy],[CreatedDate],[CarriagePerson],[LaburePerson],[OtherCharge],[CarriageCharge],[LabureCharge],[TotalPayment],PartyID,ShiftmentID)
     VALUES
          ('" + purmst.GoodsReceiveNo + "',convert(date,'" + purmst.GoodsReceiveDate + "',103),'" +
                                  purmst.PurchaseOrderNo + "',convert(date,'" + purmst.PurchaseOrderDate + "',103),"+purmst.ProjectId+",'"+purmst.ProjectSite+"','" +
                                  purmst.ChallanNo + "',convert(date,'" + purmst.ChallanDate + "',103),'" +
                                  purmst.Supplier + "','" + purmst.Remarks + "','" + purmst.TotalAmount + "','" + purmst.Discount + "','" +
                                  purmst.LoginBy + "',GETDATE(),'" + purmst.CarriagePerson + "','" + purmst.LaburePerson +
     ...
     VALUES
           ('" + PurchaseMstID + "','" + dr["ID"].ToString() + "','" + dr["item_rate"].ToString() + "','" +
            ...
            // Item stock save.......................
            if (purmst.ProjectId.Equals("Null"))
            {
                foreach (DataRow dr in dt.Rows)
                {
                    if (dr["ID"].ToString() != "" && dr["item_desc"].ToString() != "")
                    {
                        ...
                        var Qnty = dr["qnty"].ToString();
                        var Unitprice = dr["item_rate"].ToString();

                        command.CommandText = @"SELECT COUNT(*)  FROM [ItemStock] where ItemId='" + itemId + "' and ProjectId=NUll and MaterialId=NUll";
                        string StockId = command.ExecuteScalar().ToString();
                        //create Code .................
                        command.CommandText =
                            @" select Code=RIGHT(Convert(nvarchar,year(GetDate())),2)+RIGHT('000'+Convert(nvarchar,'" + itemId +
                            "'),4)+RIGHT('00000'+Convert(nvarchar,max(ID)+1),6) from ItemStock";
                        string Code = command.ExecuteScalar().ToString();

                        if (StockId.Equals("0"))
                        {
                            command.CommandText = @"INSERT INTO [ItemStock]
          (ItemPurchaseID, ItemID,Code, StyleNo, BatchNo, UnitPrice,ProjectId, Quantity,  Total, CreatedBy, CreatedDate, MsrUnitCode, Additional, Images,ColorID,SizeID,ClosingStock,ClosingAmount,ItemType,MaterialId)
     VALUES
           ('" + PurchaseMstID + "','" + itemId + "','" + Code + "','" + Code + "',NULL,'" + Unitprice + "',NUll,'" +
                                                  dr["qnty"].ToString() + "','" +
                                                  Convert.ToDouble(dr["item_rate"].ToString()) *
                                                  Convert.ToDouble(dr["qnty"].ToString()) + "','" + purmst.LoginBy +
                                                  "',GETDATE(),'" + dr["msr_unit_code"].ToString() + "','" +
                                                  dr["Additional"].ToString() + "',NULL,NULL,NULL,'" + Qnty + "','" +
                                                  Convert.ToDouble(dr["item_rate"].ToString()) *
                                                  Convert.ToDouble(dr["qnty"].ToString()) + "','1',NUll)";
                            command.ExecuteNonQuery();
                        }
                        else
                        {
                            command.CommandText = @"Update [ItemStock] set UnitPrice='" + Unitprice + "',Quantity=Quantity +'" + Qnty + "', Total+=('" + Convert.ToDouble(dr["item_rate"].ToString()) * Convert.ToDouble(dr["qnty"].ToString()) + "'),[ItemPurchaseID]='" + PurchaseMstID + "' , ClosingStock = (isnull(ClosingStock,0) + isnull('" + Qnty + "',0)), ClosingAmount = (isnull(ClosingAmount,0) + (isnull('" + Qnty + "',0)*Unitprice))" + "  where  ItemID='" + itemId + "'  and ProjectId= NULL and MaterialId=NUll and Code='"+Code+"'";
                            command.ExecuteNonQuery();
            ...
            else
            {
                foreach (DataRow dr in dt.Rows)
                {
                    if (dr["ID"].ToString() != "" && dr["item_desc"].ToString() != "")
                    {
                        ...
                        command.CommandText = @"SELECT COUNT(*)  FROM [ItemStock] where ItemId='" + itemId + "' and ProjectId='" +
                                              purmst.ProjectId + "' and MaterialId='" + RequjationId + "' ";
                        string StockId = command.ExecuteScalar().ToString();
                        //create Code .................
                        command.CommandText =
                            @" select Code=RIGHT(Convert(nvarchar,year(GetDate())),2)+RIGHT('000'+Convert(nvarchar,'" + itemId +
          ...
          (ItemPurchaseID, ItemID,Code, StyleNo, BatchNo, UnitPrice,ProjectId, Quantity,  Total, CreatedBy, CreatedDate, MsrUnitCode, Additional, Images,ColorID,SizeID,ClosingStock,ClosingAmount,ItemType,MaterialId)
     VALUES
           ('" + PurchaseMstID + "','" + itemId + "','" + Code + "','" + Code + "',NULL,'" + Unitprice + "','" +
                                                  purmst.ProjectId + "','" +
                                                  dr["qnty"].ToString() + "','" +
                                                  Convert.ToDouble(dr["item_rate"].ToString()) *
                                                  Convert.ToDouble(dr["qnty"].ToString()) + "','" + purmst.LoginBy +
                                                  ...
                                                  dr["Additional"].ToString() + "',NULL,NULL,NULL,'" + Qnty + "','" +
                                                  Convert.ToDouble(dr["item_rate"].ToString()) *
                                                  Convert.ToDouble(dr["qnty"].ToString()) + "','1','" + RequjationId + "')";
                            command.ExecuteNonQuery();
                        }
                        else
                        {
                            command.CommandText = @"Update [ItemStock] set UnitPrice='" + Unitprice + "',Quantity=Quantity +'" + Qnty + "', Total+=('" + Convert.ToDouble(dr["item_rate"].ToString()) * Convert.ToDouble(dr["qnty"].ToString()) + "'),[ItemPurchaseID]='" + PurchaseMstID + "' , ClosingStock = (isnull(ClosingStock,0) + isnull('" + Qnty + "',0)), ClosingAmount = (isnull(ClosingAmount,0) + (isnull('" + Qnty + "',0)*Unitprice))" + "  where  ItemID='" + itemId + "'  and ProjectId= '" + purmst.ProjectId + "' and MaterialId='" + RequjationId + "'";
                            command.ExecuteNonQuery();
   ...
   SET [ReceivedDate] =convert(date,'" + purmst.GoodsReceiveDate + "',103),[PO] ='" + purmst.PurchaseOrderNo + "',[PODate] =convert(date,'" + purmst.PurchaseOrderDate + "',103),ProjectId='" + purmst.ProjectId + "',ProjectSite='"+purmst.ProjectSite+"',[ChallanNo] ='" + purmst.ChallanNo + "',[ChallanDate] =convert(date,'" + purmst.ChallanDate + "',103),[SupplierID] ='" + purmst.Supplier + "',[Remarks] ='" + purmst.Remarks + "',[Total] ='" + purmst.TotalAmount + "',[Discount] ='" + purmst.Discount + "',[ModifiedBy] ='" + purmst.LoginBy + "' ,[ModifiedDate] =GETDATE(),[CarriagePerson] ='" + purmst.CarriagePerson + "',[LaburePerson] ='" + purmst.LaburePerson + "',[OtherCharge] ='" + purmst.OtherCharge + "',[CarriageCharge] ='" + purmst.CarriageCharge + "',[LabureCharge] ='" + purmst.LabureCharge + "',[TotalPayment] ='" + purmst.TotalPayment + "',ShiftmentID='" + purmst.ShiftmentID + "'  WHERE ID='" + purmst.ID + "' ";
            command.ExecuteNonQuery();
...
//    }
  ...
  FROM [ItemPurchaseMst] t1 inner join Supplier t2 on t2.ID=t1.SupplierID left join PartyInfo t3 on t3.ID=t1.PartyID  " + per + " order By t1.ID desc";
    ...
    public static DataTable GetSupplierInfo(string Supplieer)
    {
        string connectionString = DataManager.OraConnString();
        SqlConnection sqlCon = new SqlConnection(connectionString);
        string query = @" select ID,Code,Name,ContactName,Gl_CoaCode,Phone, (Code+'-'+ContactName+'-'+Mobile) as SupplierSearch from Supplier where (Code+'-'+ContactName+'-'+Mobile)=UPPER('" + Supplieer + "')";
