...
namespace OldColor
{
    public class ItemManager
    {
        ...
        public static DataTable GetItems(string criteria)
        {
          ...
          string query = @"SELECT t1.[ID],t1.[Code],[item_code],t1.ItemsType,[item_desc],t1.Brand,[msr_unit_code],t1.[UnitPrice],t1.UnitPrice as item_rate,t1.[Currency],[UMO] ,t1.BrandName FROM View_ItemSearch t1  where  upper (ItemName) = upper('" + criteria + "') and  t1.[Active]=1";
 ...
 FROM ItemStock a1 inner join [Item] t1 on a1.ItemID=t1.ID  left join UOM t2 on t2.ID=t1.UOMID left join Brand t3 on t3.ID=t1.Brand  where  upper (t1.Name + '-' + a1.Code + '-' + CONVERT(Nvarchar, a1.UnitPrice) ) = upper('" + criteria + "') and  t1.[Active]=1";
        ...
        public static DataTable GetRqNo(string RqId)
        {
            string connectionString = DataManager.OraConnString();
            SqlConnection sqlCon = new SqlConnection(connectionString);
            string query = @"Select ItemId Id, item_code,item_desc,msr_unit_code,t2.UnitPrice as item_rate,Total_Requisition,
ISNULL(AutherizQty,0) as qnty from [MaterialRequisitionDtl] as t1 inner join Item as t2 on t1.ItemId=t2.ID  where MstId='" + RqId + "'";
            DataTable dt = DataManager.ExecuteQuery(connectionString, query, "Item");
            return dt;
        }
        
        public static DataTable GetRqNo1(string RqId)
        {
            string connectionString = DataManager.OraConnString();
            SqlConnection sqlCon = new SqlConnection(connectionString);
...
//where MstId='" + RqId + "' and t3.RequisitionId='" + RqId + "' group by t1.ItemId, item_code,item_desc,msr_unit_code,Present_Site_Requiremnt,Total_Requisition";

            string query = @"select * from ( Select t1.ItemId Id, item_code,item_desc,msr_unit_code,Total_Requisition as  item_rate,t1.AutherizQty Total_Requisition, (ISNULL(t1.AutherizQty,0)-sum(ISNULL(t2.Quantity,0))) as qnty from [MaterialRequisitionDtl] as t1 inner join ItemPurOrderDtl As t2 on t1.ItemId=t2.ItemID inner join [ItemPurOrderMst] as t3 on t2.ItemOrderMstID=t3.ID where MstId='" + RqId + "' and t3.RequisitionId='" + RqId + "' group by t1.ItemId, item_code,item_desc,msr_unit_code,t1.AutherizQty,Present_Site_Requiremnt,Total_Requisition ) as Tt where tt.qnty>0";
            DataTable dt = DataManager.ExecuteQuery(connectionString, query, "Item");
            return dt;
        }
