...
namespace OldColor
{
    public class VouchManager
    {
        ...
        public static void CreateVouchMst(VouchMst vouchmst)
        {
            ...
            string query = @"insert into gl_trans_mst(vch_sys_no,fin_mon,value_date,vch_ref_no, ref_file_no,volume_no,serial_no,vch_code,trans_type,particulars,control_amt,book_name,payee,check_no,cheq_date, cheq_amnt,money_rpt_no,money_rpt_date,status,entry_user,entry_date,AUTHO_USER_TYPE) values (convert(numeric, case '" + vouchmst.VchSysNo + "' when '' then null else '" + vouchmst.VchSysNo + "' end),  '" + vouchmst.FinMon + "', convert(datetime, nullif( '" + vouchmst.ValueDate + "',''), 103),'" + vouchmst.VchRefNo + "', '" + vouchmst.RefFileNo + "', '" + vouchmst.VolumeNo + "', '" + vouchmst.SerialNo + "','" + vouchmst.VchCode + "', '" + vouchmst.TransType + "','" + vouchmst.Particulars.Replace("'", "") + "',  convert(decimal(13,2),nullif('" + vouchmst.ControlAmt.Replace(",", "") + "','') ) ,'" + vouchmst.BookName + "','" + vouchmst.Payee + "', '" + vouchmst.CheckNo + "', convert(datetime,nullif( '" + vouchmst.CheqDate + "',''),103), convert(decimal(13,2),nullif('" + vouchmst.CheqAmnt.Replace(",", "") + "','')),'" + vouchmst.MoneyRptNo + "', convert(datetime,nullif( '" + vouchmst.MoneyRptDate + "',''),103),'" + vouchmst.Status + "', '" + vouchmst.EntryUser.ToUpper() + "', convert (datetime, nullif('" + vouchmst.EntryDate + "',''),103),'" + vouchmst.AuthoUserType + "')";
...
,amount_dr as amount_dr,amount_cr as amount_cr from gl_trans_dtl a inner join gl_trans_mst b on a.vch_sys_no=b.vch_sys_no inner join UTL_USERINFO c on c.[USER_NAME]=b.ENTRY_USER " + Parameter;
        ...
        public static DataTable GetAllFixGlCodeb(string BranchId)
        {
            string connectionString = DataManager.OraConnString();
            SqlConnection sqlCon = new SqlConnection(connectionString);
            DataTable dt = null;
