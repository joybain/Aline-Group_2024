...
public class Sales
{
    public string ID, Invoice, Company, Total, Tax, Disount, GTotal, CReceive, CRefund, Date, PMethod, PMNumber, Customer, OrderStatus, Due, DvStatus, DvDate, Remarks, BankId, ChequeDate, ChequeAmount, Chk_Status, DriverName, DriverPhoneNo, CarNo, OutTime; 
	public Sales()
	{
		//
    ...
    public Sales(DataRow dr)
    {
        ...
        if (dr["DriverName"].ToString() != string.Empty) { this.DriverName = dr["DriverName"].ToString(); }   
        if (dr["DriverPhoneNo"].ToString() != string.Empty) { this.DriverPhoneNo = dr["DriverPhoneNo"].ToString(); }   
        if (dr["CarNo"].ToString() != string.Empty) { this.CarNo = dr["CarNo"].ToString(); }
        if (dr["OutTime"].ToString() != string.Empty) { this.OutTime = dr["OutTime"].ToString(); }   
